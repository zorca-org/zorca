    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Send anonymous usage data (currently to Zed's servers; ZOrca has none of its own yet)".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Send crash reports (currently to Zed's servers; ZOrca has none of its own yet)"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "ZOrca can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some(
            "Automatically mark all new projects as trusted to unlock all ZOrca's features".into(),
        ),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
...
/// ZOrca runs coding agents as terminals, so only the CLI ones are offered.
/// Chat-style in-editor agents are deliberately absent.
pub(crate) const FEATURED_AGENT_IDS: &[&str] = &["claude-acp", "codex-acp"];
...
fn render_ai_section(cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
        .gap_0p5()
        .child(Label::new("Coding Agents"))
        .child(
            Label::new("Install a CLI agent. ZOrca runs them as terminals, one per worktree.")
                .color(Color::Muted),
        )
        .child(grid)
}
