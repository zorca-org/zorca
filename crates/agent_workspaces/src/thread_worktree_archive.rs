...
use crate::worktree_metadata_store::{ArchivedGitWorktree, WorktreeMetadataStore};
...
pub async fn persist_worktree_state(root: &RootPlan, cx: &mut AsyncApp) -> Result<i64> {
    ...
    let worktree_store = cx.update(|cx| WorktreeMetadataStore::global(cx));
    let db_result = worktree_store
...
pub async fn rollback_persist(archived_worktree_id: i64, root: &RootPlan, cx: &mut AsyncApp) {
    ...
    let store = cx.update(|cx| WorktreeMetadataStore::global(cx));
...
mod tests {
    ...
    #[gpui::test]
    async fn test_remove_root_succeeds_when_only_checkout_cleanup_fails(cx: &mut TestAppContext) {
        init_test(cx);
        ...
        let task = cx.update(|cx| cx.spawn(async move |cx| remove_root(root, cx).await));
        task.await
            .expect("removing the Git registration is semantic success");
        ...
        assert!(
            !has_worktree,
            "a checkout cleanup error must not restore a Git registration that was removed"
        );
        assert!(
            fs.is_dir(worktree_path).await,
            "best-effort cleanup may leave the unregistered checkout directory behind"
        );
        workspace.read_with(cx, |_workspace, cx| {
            assert!(
                git_ui::created_worktrees::recorded_created_at(worktree_path, None, cx).is_none(),
                "the created-worktree record must be forgotten after semantic removal"
            );
        });
    }
}
