...
impl Terminal {
    ...
    /// Makes a display-only terminal read back as a *running* task terminal, so
    /// a caller that identifies tabs by their spawn task can be tested without
    /// a pty.
    #[cfg(any(test, feature = "test-support"))]
    pub fn set_running_task_for_test(&mut self, id: task::TaskId) {
...
mod tests {
    ...
    #[gpui::test]
    async fn test_ade_session_suppresses_focus_in_out_reports(cx: &mut TestAppContext) {
        let builder = cx.update(|cx| {
            TerminalBuilder::new_display_only(
                SettingsCursorShape::Block,
                AlternateScroll::On,
