...
const DOCS_URL: &str = "https://github.com/ifokeev/zorca";
...
pub fn init(cx: &mut App) {
    // ADE fork: the workspace sidebar's actions, registered before any panel.
    ade_workspaces::init(cx);
    ...
    cx.on_action(|_: &zed_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            // `licenses.md` is generated by the packaging scripts, so it is
            // absent from development builds. Read it without `asset_str`,
            // which expects the asset to exist and takes the whole app down
            // with it when it does not.
            let licenses = Assets::get("licenses.md").and_then(|file| match file.data {
            ...
            let Some(licenses) = licenses else {
                workspace.show_error(
                    "Dependency licenses are only bundled into packaged builds, \
                     so there is nothing to show in this one.",
                    cx,
                );
                return;
            };
            ...
            window_handle
                .update(cx, |_, window, cx| {
                    ...
                    multi_workspace_handle.update(cx, |multi_workspace, cx| {
                        ...
                        // ZOrca shows the worktree sidebar by default. Done here rather
                        // than in the MultiWorkspace constructor, which runs before the
                        // sidebar entity exists.
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        // Loaded but deliberately not added as a dock: ZOrca puts terminals in
        // the centre. The panel survives only as the workspace's terminal
        // provider, which is what task and debugger spawning goes through.
        let terminal_panel = TerminalPanel::load(workspace_handle.clone(), cx.clone());
        let git_panel = GitPanel::load(workspace_handle.clone(), cx.clone());
        let debug_panel = DebugPanel::load(workspace_handle.clone(), cx);
        let ports_panel = recent_projects::PortsPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            async {
                terminal_panel.await.context("failed to load terminal panel").log_err();
            },
...
fn open_about_window(cx: &mut App) {
    fn about_window_icon(icon: settings::AppIcon, release_channel: ReleaseChannel) -> Arc<Image> {
        let bytes = app_icon_bytes(icon, release_channel);
    ...
    impl AboutWindow {
        fn new(cx: &mut Context<Self>) -> Self {
            ...
            Self {
                ...
                app_icon: about_window_icon(AppIconSetting::get_global(cx).0, release_channel),
                message,
                commit,
                full_version,
            }
...
#[derive(Copy, Clone, Debug, settings::RegisterSetting)]
struct AppIconSetting(settings::AppIcon);

impl Settings for AppIconSetting {
    fn from_settings(content: &settings::SettingsContent) -> Self {
        Self(content.app_icon.unwrap_or_default())
    }
}

fn app_icon_bytes(icon: settings::AppIcon, release_channel: ReleaseChannel) -> &'static [u8] {
    match icon {
        settings::AppIcon::Default => match release_channel {
        ...
        settings::AppIcon::Classic => include_bytes!("../resources/app-icon.png"),
        settings::AppIcon::Graphite => include_bytes!("../resources/app-icon-dev.png"),
        settings::AppIcon::Aurora => include_bytes!("../resources/app-icon-preview.png"),
        settings::AppIcon::Neon => include_bytes!("../resources/app-icon-nightly.png"),
    }
}

fn init_application_icon(cx: &mut App) {
    let apply = |cx: &mut App| {
        cx.set_application_icon(app_icon_bytes(
            AppIconSetting::get_global(cx).0,
...
/// Actions inside `AI_ACTION_NAMESPACES` that reach ZOrca's terminal agents rather than
/// a model, so they keep their bindings when `disable_ai` is set.
const TERMINAL_ACTION_ALLOWLIST: &[&str] = &[
    "agent::NewTerminalThread",
    "agent::ToggleNewThreadMenu",
    "agent::ToggleOptionsMenu",
];

fn is_ai_keybinding(binding: &KeyBinding) -> bool {
    let name = binding.action().name();
    if TERMINAL_ACTION_ALLOWLIST.contains(&name) {
        return false;
    }
...
mod tests {
    ...
    async fn test_editor_zoom_with_scroll_wheel(cx: &mut TestAppContext) {
        ...
        // ZOrca opens the sidebar at startup; close it so the hardcoded mouse position
        // below lands on the editor rather than on the sidebar.
        cx.run_until_parked();
        window
    ...
    /// The actions a single default keymap asset resolves for `keystroke` in `context`,
    /// highest priority first.
    fn default_keymap_bindings_for(
        asset: &str,
        keystroke: &str,
        context: &str,
        cx: &mut TestAppContext,
    ...
    /// ctrl-j is the terminal's own keystroke (accept/newline in TUIs) but also
    /// toggles the bottom dock. A focused terminal must win; everywhere else the
    /// dock binding must survive.
    #[gpui::test]
    fn test_ctrl_j_reaches_a_focused_terminal(cx: &mut TestAppContext) {
        init_keymap_test(cx);

        for asset in ["keymaps/default-linux.json", "keymaps/default-windows.json"] {
            let in_terminal =
                default_keymap_bindings_for(asset, "ctrl-j", "Workspace Terminal", cx);
            assert_eq!(
                in_terminal.first(),
                Some(&"terminal::SendKeystroke".to_string()),
                "{asset}: ctrl-j in a terminal should be sent to the shell, got {in_terminal:?}"
            );

            let outside = default_keymap_bindings_for(asset, "ctrl-j", "Workspace", cx);
            assert_eq!(
                outside.first(),
                Some(&"workspace::ToggleBottomDock".to_string()),
                "{asset}: ctrl-j outside a terminal should toggle the bottom dock, got {outside:?}"
            );
        }
    }
    ...
    async fn test_disable_ai_filters_keybindings(cx: &mut gpui::TestAppContext) {
        ...
        let user_binding =
            KeyBinding::new("ctrl-enter", zed_actions::agent::ToggleModelSelector, None);
    ...
    #[gpui::test]
    async fn test_disable_ai_keeps_terminal_agent_bindings(cx: &mut gpui::TestAppContext) {
        let _app_state = init_keymap_test(cx);
        ...
        let terminal_binding =
            KeyBinding::new("ctrl-enter", agent_workspaces::NewTerminalThread, None);
        cx.update(|cx| reload_keymaps(cx, vec![terminal_binding]));
        ...
        cx.update(|cx| {
            ...
            assert!(
                keymap
                    .bindings()
                    .any(|binding| binding.action().name() == "agent::NewTerminalThread"),
                "terminal-agent bindings must survive disable_ai"
            );
            if let Some(binding) = keymap
                .bindings()
                .find(|binding| binding.action().name() == "agent::ToggleModelSelector")
            {
                panic!(
                    "model-backed agent bindings must still be filtered, but found `{}`",
                    binding.action().name()
                );
            }
        });
    ...
    #[gpui::test]
    async fn test_same_process_reopen_restores_current_session(cx: &mut TestAppContext) {
        use session::Session;
        use workspace::{OpenMode, Workspace};

        let app_state = init_test(cx);
        let stale_dir = path!("/stale_session");
        let current_dir = path!("/current_session");
        let fake_fs = app_state.fs.as_fake();
        fake_fs.insert_tree(stale_dir, json!({})).await;
        fake_fs.insert_tree(current_dir, json!({})).await;

        let stale_session_id = cx.read(|cx| app_state.session.read(cx).id().to_owned());
        let workspace::OpenResult {
            window: stale_window,
            ..
        } = cx
            ...
            .await
            .expect("failed to open stale-session workspace");
        flush_workspace_serialization(&stale_window, cx).await;
        stale_window
            .update(cx, |_, window, _| window.remove_window())
            .expect("failed to close stale-session window");
        cx.run_until_parked();

        cx.update(|cx| {
            app_state.session.update(cx, |app_session, _cx| {
                app_session.replace_session_for_test(Session::test_with_old_session(
        ...
        let workspace::OpenResult {
            ...
            .await
            .expect("failed to open current-session workspace");
        flush_workspace_serialization(&current_window, cx).await;
        current_window
            .update(cx, |_, window, _| window.remove_window())
            .expect("failed to close current-session window");
        cx.run_until_parked();

        let mut async_cx = cx.to_async();
        crate::reopen_or_create_workspace(app_state, &mut async_cx)
            .await
        ...
        restored_windows[0]
            .read_with(cx, |multi_workspace, cx| {
                let roots = multi_workspace.workspace().read(cx).root_paths(cx);
                assert!(
                    roots
                        .iter()
                        .any(|path| path.as_ref() == Path::new(current_dir)),
                    "the current session workspace should be restored"
                );
                assert!(
                    !roots
                        .iter()
                        .any(|path| path.as_ref() == Path::new(stale_dir)),
                    "the previous session workspace must not be restored"
                );
            })
            .expect("failed to read restored workspace");
    }
